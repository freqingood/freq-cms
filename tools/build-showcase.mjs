...
/* ─────────────────────────────────────────────────────────────
   ...
   Scans /profiles for audio files and writes /showcase.json.
...
const OUT_FILE = path.join(ROOT, 'showcase.json');

const AUDIO_EXT = new Set(['.mp3', '.m4a', '.aac', '.ogg', '.opus', '.flac', '.wav', '.webm']);

/* Root-relative, because the site is served from the apex of a custom
   domain (thefreq.in). A leading slash keeps the paths correct no
   matter which page ends up embedding the showcase. */
const BASE = '/profiles/';
...
async function build() {
  ...
  console.log(`showcase.json written — ${profiles.length} profile(s)`);
